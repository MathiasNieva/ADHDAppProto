...
import com.example.adhdappprototype.ui.comprehensive_todo_list.ComprehensiveTodoListViewModel
            ...
            when(event) {
                is UiEvent.ShowSnackbar -> {
                    ...
                    if(result == SnackbarResult.ActionPerformed) {
                        viewModel.onEvent(DailyPlannerEvent.OnUndoDeleteClick)
                    }
                }
    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(onClick = {
                viewModel.onEvent(DailyPlannerEvent.OnAddTodoClick)
            }) {
                Icon(
            ...
            items(todos.value) { todo ->
                DailyPlanItem(
                    ...
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable {
                            viewModel.onEvent(DailyPlannerEvent.OnTodoClick(todo))
                        }
                        .padding(16.dp)
                )
            }
